...
int main(int argc, char * argv[]){
  ...
  const double nedm95 = 1.6e-26;
  const double pedm95 = 2.0e-25;
  ...
  if (opt == 1){//for SoLID
    ...
    efx = efx1;//neutron
    //KPSY15
    double par1[9] = {0.0, 0.0, 0.413, -0.229, 0.0, nedm95/Xscale, 0.133, 0.094, 0.0};
    for (int i = 0; i < 1000; i++){
      y[i] = fx(&x[i], par1);
      err[i] = efx(&x[i], par1);
    }
    ...
    //SoLID
    double par2[9] = {0.0, 0.0, 0.413, -0.229, 0.0, nedm95/Xscale, 0.018, 0.008, 0.0};//SoLID
    for (int i = 0; i < 1000; i++){
      y[i] = fx(&x[i], par2);
      err[i] = efx(&x[i], par2);
    }
    ...
    //SoLID + nEDM
    double par3[9] = {0.0, 0.0, 0.413, -0.229, 0.0, 0.01 * nedm95/Xscale, 0.018, 0.008, 0.0};//SoLID
    for (int i = 0; i < 1000; i++){
      y[i] = fx(&x[i], par3);
      err[i] = efx(&x[i], par3);
    }
  ...
  if (opt == 2){
    ...
    efx = efx2;
    //KPSY15 + pEDM
    double par4[9] = {0.0, 0.0, 0.413, -0.229, 0.0, pedm95/Xscale, 0.133, 0.094, 0.0};
    for (int i = 0; i < 1000; i++){
      y[i] = fxp(&x[i], par4);
      err[i] = efx(&x[i], par4);
    }
    ...
    //SoLID + pEDM
    double par5[9] = {0.0, 0.0, 0.413, -0.229, 0.0, pedm95/Xscale, 0.018, 0.008, 0.0};//SoLID
    for (int i = 0; i < 1000; i++){
      y[i] = fxp(&x[i], par5);
      err[i] = efx(&x[i], par5);
    }
