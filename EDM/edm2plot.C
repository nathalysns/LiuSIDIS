...
int main(int argc, char * argv[]){
  ...
  const double pedm95 = 5.4e-24 / Xscale;
  ...
  if (opt == 1){//for SoLID
    ...
    efx = efx2;
    //KPSY15 + pEDM
    double par4[9] = {0.0, 0.0, 0.413, -0.229, 0.0, pedm95, 0.133, 0.094, 0.0};
    for (int i = 0; i < 1000; i++){
      y[i] = fxp(&x[i], par4);
      err[i] = efx(&x[i], par4);
    }
    ...
    //SoLID + pEDM
    double par5[9] = {0.0, 0.0, 0.413, -0.229, 0.0, pedm95, 0.018, 0.008, 0.0};//SoLID
    for (int i = 0; i < 1000; i++){
      y[i] = fxp(&x[i], par5);
      err[i] = efx(&x[i], par5);
    }
    ...
    l0->AddEntry(g4, "#font[22]{KPSY2016 + current pEDM}", "f");
    l0->AddEntry(g5, "#font[22]{SoLID + current pEDM}", "f");
    g4->DrawClone("3same");
    g5->DrawClone("3same");
    //g1->DrawClone("3same");
    //g2->DrawClone("3same");
    ...
    efx = efx1;
    double par6[9] = {0.0, 0.0, 0.413, -0.229, 0.0, nedm90, 0.0, 0.0, 0.0};//SoLID
    for (int i = 0; i < 1000; i++){
      y[i] = fx(&x[i], par6);
      err[i] = efx(&x[i], par6);
      err1[i] = y[i] + err[i];
