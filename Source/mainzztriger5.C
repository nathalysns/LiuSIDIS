...
double GetAcceptance_new(const TLorentzVector p, const char * part){//
  double theta = p.Theta() / M_PI * 180.0;
  if (theta < 8.0) return 0;
  double mom = p.P();
  double acc = 0;
...
int main(int argc, char * argv[]){
  ...
  for (Long64_t i = 0; i < Nsim; i++){
    ...
    if (weight > 0){
      ...
      acc_new = GetAcceptance_new(lp, "e-") * GetAcceptance_new(Ph, argv[1]);
      ...
      sx->Fill(x, weight * acc_new);
      sz->Fill(z, weight * acc_new);
      sPt->Fill(Pt, weight * acc_new);
      sxz->Fill(x, z, weight * acc_new);
      sxPt->Fill(x, Pt, weight * acc_new);
      szPt->Fill(z, Pt, weight * acc_new);
