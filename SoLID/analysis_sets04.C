...
int main(int argc, char * argv[]){
  if (argc < 3){
    cout << "./analysis_world <R0> <readfile>" << endl;
    return 0;
  }
  ...
  string filename = "sets04/" + sets + ".csv";
  string writename = "sets05/" + sets + ".csv";
  ...
  string obs, target, hadron, experiment, dependence;
   
  FILE * file = fopen(writename.data(), "w");
  fprintf(file, "%s\n", tmp.data());
  ...
  while (infile.good()){
    ...
    fprintf(file, "%d,%.1f,%.6f,%.6f,%.6f,%.6f,%.6f,%s,%.6f,%.6f,%.6f,%.6f,%s,%s,%s,%s\n",
	    j++, Ebeam, x, y, z, Q2, pT, obs.data(), value, stat, systrel, systabs, target.data(), hadron.data(), experiment.data(), dependence.data());
  }
