...
class Lsidis{
 ...
 private:
  ...
  int hio;//fragmentation order
  int dssih;//hadron type for DSS fragmentation
...
int Lsidis::SetHadron(const char * hadron){//set final hadron
  if (strcmp(hadron, "pi+") == 0){
    ...
    dssih = 1; hic = 0; hid = 210;
    Mh = Mpi0;
    MXminp = Mp;
    MXminn = Mp;
  }
  else if (strcmp(hadron, "K+") == 0){
    dssih = 2; hic = 1; hid = 321;
    Mh = Mkaon;
    MXminp = MLambda;
    MXminn = MSigmam;
  }
    ...
    dssih = 2; hic = 0; hid = 310;
    Mh = MK0;
    MXminp = MSigmap;
    MXminn = MLambda;
  }
  else if (strcmp(hadron, "p") == 0){
    dssih = 3; hic = 1; hid = 2212;
    Mh = Mp;
    MXminp = Mpi0;
    MXminn = Mpion;
  }
...
int Lsidis::GetFFs(){//get FFs to hadron at current z, Q2
  if (!st_hadron){
    perror("No hadron type initialized!");
    return -1;
  }
  if (physics_control){
    double u, ubar, d, dbar, s, sbar, c, b, g;
    fdss_ (&dssih, &hic, &hio, &z, &Q2, &u, &ubar, &d, &dbar, &s, &sbar, &c, &b, &g);
    D1[0] = u / z;//u
    D1[1] = d / z;//d
    D1[2] = s / z;//s
    D1[3] = ubar / z;//ubar
